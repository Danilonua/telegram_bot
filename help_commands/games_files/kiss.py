...
async def send_kiss(message: types.Message):
    try:
        ...
        await message.answer(f'[{your_name}](tg://user?id={str(your_id)}) поцеловал/а [{friend_name}](tg:'
                             f'//user?id={str(friend_id)})', parse_mode="Markdown")
    except AttributeError:
        your_id = message.from_id
        your_name = message.from_user.username
        await message.answer(f'[{your_name}](tg://user?id={str(your_id)}) поцеловал/а всех', parse_mode="Markdown")


def register_handlers_kiss_command(dp: Dispatcher):
